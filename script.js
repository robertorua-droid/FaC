...
// XML
    ...
    function generateInvoiceXML(invoiceId) {
        ...
        if (!invoice) { alert("Errore!"); return; }

        const company = getData('companyInfo');
        const customer = getData('customers').find(c => String(c.id) === String(invoice.customerId)) || {};

        // -------------------------------
        // 1. DATI NUMERICI DI BASE
        // -------------------------------
        const importoBollo = safeFloat(invoice.importoBollo || 0);              // es. 2.00
        const totalePrestazioni = safeFloat(invoice.totalePrestazioni || invoice.totaleImponibile || 0); // es. 1185.00
        const rivalsaImporto = (invoice.rivalsa && typeof invoice.rivalsa.importo !== 'undefined')
            ? safeFloat(invoice.rivalsa.importo)
            : 0;                                                                 // es. 47.40
        const rivalsaPerc = safeFloat(company.aliquotaInps || 0);               // es. 4.00

        // -------------------------------
        // 2. ANAGRAFICA CEDENTE
        // -------------------------------
        ...
        // Uppercase per Comune/Provincia (richiesto da FatturaPA / FatturaCheck)
        const indirizzoCed = company.address || '';
        const civicoCed = company.numeroCivico || '';
        const comuneCed = (company.city || '').toUpperCase();
        const provinciaCed = (company.province || '').toUpperCase();
        const capCed = company.zip || '';

        const indirizzoCli = customer.address || '';
        const comuneCli = (customer.comune || '').toUpperCase();
        const provinciaCli = (customer.provincia || '').toUpperCase();
        const capCli = customer.cap || '';
        const sdi = (customer.sdi && customer.sdi.trim() !== '') ? customer.sdi.trim() : '0000000';

        // Se il Codice Fiscale cliente è vuoto, usa la P.IVA come fallback
        const cfCliente = (customer.codiceFiscale && customer.codiceFiscale.trim() !== '')
            ? customer.codiceFiscale.trim()
            : (customer.piva || '');

        // -------------------------------
        // 3. RIEPILOGO IVA / NATURA
        // -------------------------------
        // Regola: DatiRiepilogo per ogni combinazione (AliquotaIVA, Natura)
        //         con N4 che somma Rivalsa INPS + (eventuale) bollo.
        const summaryMap = {};

        function addToSummary(aliquota, natura, imponibile, calcolaIvaSuStessoImporto = false) {
            const aliq = safeFloat(aliquota);
            const nat = natura || '';
            const key = `${aliq.toFixed(2)}|${nat}`;
            if (!summaryMap[key]) {
                summaryMap[key] = { aliquota: aliq, natura: nat, imponibile: 0, imposta: 0 };
            }
            const imp = safeFloat(imponibile);
            summaryMap[key].imponibile += imp;
            if (calcolaIvaSuStessoImporto && aliq > 0) {
                summaryMap[key].imposta += imp * aliq / 100;
            }
        }

        // 3.a: Linee di dettaglio (beni/servizi)
        invoice.lines.forEach(l => {
            const qty = safeFloat(l.qty);
            const price = safeFloat(l.price);
            const subtotal = (typeof l.subtotal !== 'undefined') ? safeFloat(l.subtotal) : qty * price;
            const aliq = safeFloat(l.iva);
            const desc = (l.productName || '').toLowerCase();

            // Natura in base alla logica FatturaPA:
            // - Se IVA = 0, la Natura è obbligatoria.
            // - Per la riga "Rivalsa bollo" forziamo N4 (come nel file AE che hai allegato).
            let naturaLinea = '';
            if (aliq === 0) {
                if (desc.includes('rivalsa bollo')) {
                    naturaLinea = 'N4';
                } else if (l.esenzioneIva) {
                    naturaLinea = l.esenzioneIva;
                }
            }

            // Riepilogo: la base imponibile è il PrezzoTotale della linea
            addToSummary(aliq, naturaLinea, subtotal, true);
        });

        // 3.b: Aggiungi la RIVAlsa INPS alla natura N4
        if (rivalsaImporto > 0) {
            addToSummary(0, 'N4', rivalsaImporto, false);
        }

        // NB: l'importo del bollo NON viene aggiunto di nuovo qui, perché è già presente
        // come riga "Rivalsa bollo" (che mappiamo a Natura N4). In questo modo:
        //   Imponibile N4 = Rivalsa INPS + Bollo
        //   e rispettiamo la logica del file AE (hm14v).

        let riepilogoXml = '';
        Object.values(summaryMap).forEach(s => {
            const aliqStr = s.aliquota.toFixed(2);
            const imponibileStr = s.imponibile.toFixed(2);
            const impostaStr = (s.aliquota > 0 ? (s.imponibile * s.aliquota / 100) : 0).toFixed(2);

            riepilogoXml += `<DatiRiepilogo><AliquotaIVA>${aliqStr}</AliquotaIVA>`;
            if (aliqStr === '0.00' && s.natura) {
                riepilogoXml += `<Natura>${escapeXML(s.natura)}</Natura>`;
            }
            riepilogoXml += `<ImponibileImporto>${imponibileStr}</ImponibileImporto><Imposta>${impostaStr}</Imposta></DatiRiepilogo>`;
        });
        ...
        let xml = `<?xml version="1.0" encoding="UTF-8"?>` +
            ...
            `<DatiTrasmissione>` +
                ...
                `<CodiceDestinatario>${escapeXML(sdi)}</CodiceDestinatario>` +
            `</DatiTrasmissione>` +
            ...
            `<CedentePrestatore>` +
                ...
                `</DatiAnagrafici>` +
                `<Sede>` +
                    `<Indirizzo>${escapeXML(indirizzoCed)}</Indirizzo>` +
                    `<NumeroCivico>${escapeXML(civicoCed)}</NumeroCivico>` +
                    `<CAP>${escapeXML(capCed)}</CAP>` +
                    `<Comune>${escapeXML(comuneCed)}</Comune>` +
                    `<Provincia>${escapeXML(provinciaCed)}</Provincia>` +
                    `<Nazione>IT</Nazione>` +
                `</Sede>` +
            `</CedentePrestatore>` +
            ...
            `<CessionarioCommittente>` +
                `<DatiAnagrafici>` +
                    ...
                    `<CodiceFiscale>${escapeXML(cfCliente)}</CodiceFiscale>` +
                    `<Anagrafica><Denominazione>${escapeXML(customer.name || '')}</Denominazione></Anagrafica>` +
                `</DatiAnagrafici>` +
                `<Sede>` +
                    `<Indirizzo>${escapeXML(indirizzoCli)}</Indirizzo>` +
                    `<CAP>${escapeXML(capCli)}</CAP>` +
                    `<Comune>${escapeXML(comuneCli)}</Comune>` +
                    `<Provincia>${escapeXML(provinciaCli)}</Provincia>` +
                    `<Nazione>IT</Nazione>` +
                `</Sede>` +
            `</CessionarioCommittente>` +
            `</FatturaElettronicaHeader>` +
        ...
        // Dati Bollo (se presente)
        if (importoBollo > 0) {
            xml += `<DatiBollo><BolloVirtuale>SI</BolloVirtuale><ImportoBollo>${importoBollo.toFixed(2)}</ImportoBollo></DatiBollo>`;
        }

        // Dati Cassa Previdenziale (se presente)
        if (rivalsaImporto > 0 && totalePrestazioni > 0) {
            xml += `<DatiCassaPrevidenziale>` +
                `<TipoCassa>TC22</TipoCassa>` +
                `<AlCassa>${rivalsaPerc.toFixed(2)}</AlCassa>` +
                `<ImportoContributoCassa>${rivalsaImporto.toFixed(2)}</ImportoContributoCassa>` +
        ...
        // Causale per Nota di Credito
        if (invoice.type === 'Nota di Credito' && invoice.reason) {
            xml += `<Causale>${escapeXML(invoice.reason)}</Causale>`;
        }

        xml += `</DatiGeneraliDocumento></DatiGenerali>`;

        // -------------------------------
        // 5. DETTAGLIO LINEE E RIEPILOGO
        // -------------------------------
        xml += `<DatiBeniServizi>`;
        ...
        invoice.lines.forEach(l => {
            ...
            xml += `<DettaglioLinee>` +
                ...
                `<PrezzoTotale>${subtotal.toFixed(2)}</PrezzoTotale>` +
                `<AliquotaIVA>${aliq.toFixed(2)}</AliquotaIVA>`;

            if (aliq === 0 && naturaLinea) {
                xml += `<Natura>${escapeXML(naturaLinea)}</Natura>`;
            }
        ...
        // -------------------------------
        // 7. DOWNLOAD FILE (nome univoco)
        // -------------------------------
        const randomSuffix = (Math.random().toString(36) + '00000').slice(2, 7);
